...
drops = {
    20: [
        {'auto_target': True, 'target_edge': True, 'height': 10, 'width': 5, 'water_intensity': 0.8, 'cooling_effect': 0.8}
    ],
    40: [
        {'auto_target': True, 'target_edge': True, 'height': 10, 'width': 15, 'water_intensity': 0.5, 'cooling_effect': 0.8}
    ],
    50: [
        {'auto_target': True, 'target_edge': True, 'height': 8, 'width': 6, 'water_intensity': 0.7, 'cooling_effect': 0.8}
    ]
}
...
#### Fifth Test: Firebreak (targets wind direction)
params.start_cells = [(50, 50)]
params.wind = (0.5, 0.5)
params.wind_strength_factor = 0.6
...
firebreaks_dynamic = {
    15: [
        {'auto_target': True, 'wind_offset': 30, 'height': 80, 'width': 10}
    ]
}

T = 80 
smart_firebreak_sim = FireSpreadingAdvanced(params)
smart_firebreak_sim.run_simulation(T, gif_name="results/test5_smart_firebreak", scheduled_firebreaks=firebreaks_dynamic)

#### Sixth Test: Simulation with moisture
...
water_fire.run_simulation(T, gif_name="results/test6_water_barrier")
